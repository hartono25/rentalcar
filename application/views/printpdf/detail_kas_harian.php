    ...
    <table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" style="margin-bottom: 20px;">
        ...
        <tbody>
            ...
            <?php foreach ($list as $l) : ?>
                ...
                <tr class="style6b">
                    <td colspan="5" style="padding-left: 10px;" class="garisbawah"><?= Tanggal(date('Y-m-d', strtotime($l['p_tanggal']))); ?></td>
                </tr>
                <?php $data = $this->Model->get_kas_by_tgl(strtotime($l['p_tanggal']));
                $no = 0;
                $tkredit = 0;
                $tdebet = 0; ?>
                <?php foreach ($data as $d) : ?>
                    <?php $tkredit += $d['kredit'];
                    $tdebet += $d['debet']; ?>
                    <tr class="style9">
                        <td style="text-align: center;" class="garisbawah"><?= ($no + 1) . '.'; ?></td>
                        <td style="padding-left: 10px;" class="garisbawah"><?= $d['jenis']; ?></td>
                        <td style="text-align: right; padding-right: 10px;" class="garisbawah"><?= rupiah($d['debet']); ?></td>
                        <td style="text-align: right; padding-right: 10px;" class="garisbawah"><?= rupiah($d['kredit']); ?></td>
                        <td style="padding-left: 10px;" class="garisbawah"><?= $d['keterangan']; ?></td>
                    </tr>
                    <?php $no++; ?>
                <?php endforeach; ?>
                <tr>
